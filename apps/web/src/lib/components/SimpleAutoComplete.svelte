...
<div
    ...
    {multiple ? 'is-multiple' : ''} autocomplete is-fullwidth {uniqueId} h-12"
	class:show-clear={clearable}
	class:is-loading={showLoadingIndicator && loading}
>
	<select name={selectName} id={selectId} {multiple} class="select select-bordered h-12">
		{#if !multiple && value}
